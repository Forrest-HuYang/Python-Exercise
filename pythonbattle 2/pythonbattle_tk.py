...
class BattleGraphicsEngine(GraphicsEngine):
    def __init__(self, width, height):
        self.bot1img = None
        self.bot2img = None
        super().__init__(width, height)
        self.root.title("Python AI battle")

    def loadBotImage(self, bot1img, bot2img):
        self.bot1img = Image.open(bot1img)
        self.bot2img = Image.open(bot2img)

    def drawLinePygame(self, rgb, start_pos, end_pos, line_width=1):
        self.drawLine(
            self._from_rgb(rgb), start_pos[0], start_pos[1], end_pos[0], end_pos[1], line_width)

    def drawRectPygame(self, rgb,  rect, line_width=0):
            # rect [ ( top, lef, ), (width, height) ]
        self.drawRect(self._from_rgb(rgb), rect[0][0],
                      rect[0][1], rect[1][1], rect[1][0], line_width)

    def drawTextPygame(self, text, fg_rgb, pos, font_size=20):
        self.drawText( self._from_rgb(fg_rgb), pos[0], pos[1], text, font_size )

    def drawBot1ImagePygame(self, pos, rotation):
        self.bot1imgTk = ImageTk.PhotoImage(self.bot1img.rotate(rotation))
        self.drawImage(self.bot1imgTk, pos[0], pos[1])

    def drawBot2ImagePygame(self, pos, rotation):
        self.bot2imgTk = ImageTk.PhotoImage(self.bot2img.rotate(rotation))
        self.drawImage(self.bot2imgTk, pos[0], pos[1])

    def drawBattlefield(self, bot1, bot2):
        """draws the battlefield with Pygame"""
        global state, redsquares, bluesquares, ai1name, ai2name

        # Clear the screen
        self.canvas.delete("all")

        self.drawRectPygame((0, 0, 0), ((480,0), (160, 480)) )

        # Draw colored squares
        for i in redsquares:
            self.drawRectPygame(
                (64, 0, 0), (((i[0]-1)*48, (i[1]-1)*48), (48, 48)))

        for i in bluesquares:
            self.drawRectPygame(
                (0, 0, 64), (((i[0]-1)*48, (i[1]-1)*48), (48, 48)))

        # draw grid lines
        for i in range(1, 10):
            self.drawLinePygame((50, 50, 50), (0, i*48), (480, i*48), 5)
            self.drawLinePygame((50, 50, 50), (i*48, 0), (i*48, 480), 5)
        ...
        ge.drawBot1ImagePygame(bot1pos, -90*bot1.rotation)
        ge.drawBot2ImagePygame(bot2pos, -90*bot2.rotation)

        # Write the names of the robots on the screen
        self.drawTextPygame(ai1name, (255, 0, 0), (480, 0), 16)
        self.drawTextPygame(ai2name, (0, 0, 255), (480, 40), 16)
        
        # Draw the robot's health bars
        self.drawRectPygame((0, 255, 0), ((480, 20),
                                    (int(bot1.health*160/100.0), 10)))
        self.drawRectPygame((0, 255, 0), ((480, 60),
                                    (int(bot2.health*160/100.0), 10)))
...
######################################
ge = BattleGraphicsEngine(640, 480)

# Create Font object
# Get names of competitors
# ai1name = raw_input("Enter red AI: ")
ai1name = "randomai" #input("Enter red AI: ")
# ai2name = raw_input("Enter blue AI: ")
ai2name = "circleai" #input("Enter blue AI: ")
...
ge.loadBotImage("DozerRed.png", "DozerBlue.png")
ge.drawBattlefield(field[0], field[1])
...
if state == "stalemate":
    ...
    ge.drawBattlefield(field[0], field[1])  # ASCII/PYGAME
